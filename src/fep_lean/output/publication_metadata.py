...
PDF_KEYWORD_SEPARATOR = "; "
_HYPERSETUP_FIELD_RE = re.compile(
    r"^\s*(?P<field>pdfsubject|pdfkeywords)=\{(?P<value>[^{}]*)\}",
    re.MULTILINE,
)


def pdf_metadata_fields(project_root: Path) -> dict[str, str]:
    """Return the PDF ``/Info`` strings ``manuscript/config.yaml`` declares.

    ``paper.subtitle`` and ``keywords`` are authored in ``config.yaml``, but the
    combined PDF's ``\\hypersetup`` is generated by pandoc, which carries only
    title, author and language. The two strings therefore have to be written
    into ``manuscript/preamble.md`` by hand -- the renderer copies that file
    verbatim and never substitutes ``{{placeholders}}`` in it. This function is
    ...
    return {
        "pdfsubject": subtitle,
        "pdfkeywords": PDF_KEYWORD_SEPARATOR.join(keywords),
    }


def pdf_metadata_drift(project_root: Path) -> tuple[str, ...]:
    """Return one message per PDF metadata string that no longer matches config.

    A published PDF that silently drops or misstates its own subject and keyword
    list is not reproducible from its declared metadata, and the copy in the
    preamble cannot be checked by the placeholder gate. Returning the drift lets
    the render entry point fail closed before the PDF is built.
    """
    ...
    try:
        ...
        raise PublicationMetadataError(
            f"cannot read manuscript/preamble.md: {exc}"
        ) from exc
    found = {
        match.group("field"): match.group("value").strip()
        for match in _HYPERSETUP_FIELD_RE.finditer(text)
    }
    drift: list[str] = []
