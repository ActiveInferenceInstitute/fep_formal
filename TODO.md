...
| FEP-EVIDENCE-CURRENT | Refresh the remaining deterministic evidence planes for the accepted post-v1.1.0 source wave against the settled current owner roster (bridge contract v0.6 with the GNN 3.3.0 route rename re-pin). On 2026-09-09 `fep-lean status` verified the publication/manuscript drift plane current (render receipt, fonts receipt, and all catalogue projections byte-current) while the other planes still bind pre-status-verb owner bytes: `bridge_source_pin` is stale (`src/fep_lean/cli.py`, `src/fep_lean/output/render_fonts.py` changed after the recorded fep_lean commit) and `native_verification_receipt` is `not_claim_ready` (155/155 topics compiled warning- and sorry-free at the recorded digest, but `source_digest` no longer matches the live tree). Remaining work: re-pin the bridge custody, then refresh the native, declaration/axiom (formal-audit), Python-acceptance (Horizon chain), and browser-replay receipts against the live roster digest. The predecessor receipts bind `pyproject.toml` and `uv.lock` bytes, so a dependency refresh alone invalidates the recorded Horizon acceptance chain and must be re-pinned together with the evidence refresh. Retained receipts from an older owner manifest or source digest remain historical, never current evidence. | Native validation is `valid`, `source_bound`, and `native_claim_ready`; formal-audit, Python-acceptance, and browser-replay validators return no errors; manuscript/publication drift checks pass; and a deterministic release bundle validates as source-bound and claim-ready against the same final owner roster. |
