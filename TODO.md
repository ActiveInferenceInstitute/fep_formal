...
| FEP-EVIDENCE-CURRENT | Refresh the remaining deterministic evidence planes for the accepted post-v1.1.0 source wave against the settled current owner roster (owner manifest v17). On 2026-09-11 the final refresh chain completed against the wave-2 HEAD (3ee3007): bridge custody re-pinned (`fep-lean bridge pin` at 62735b1, `bridge_source_pin` current), native verification receipt regenerated (155/155 warning- and sorry-free; `validate_native_lean_receipt(receipt, project_root=...)` reports `valid`/`source_bound`/`native_claim_ready` — formal two-arg call), formalism-audit receipt refreshed with zero errors, publication/manuscript plane current (catalogue, render-fonts, and the strict-default `render_manuscript --check` all green), and the browser acceptance receipt re-captured with `_browser_receipt_errors` empty. `fep-lean status` reads all four sections current with the top-level `native_claim_ready` flag honestly derived from the section state (VI-7 fix). **Remaining plane: Python-acceptance.** The hermetic acceptance environment (`_TEST_COLLECTION_ENVIRONMENT_POLICY`: PATH stripped, plugin autoload off) cannot run the tool-dependent lanes — serial_lean probes need lake/lean, render-fonts tests need fontconfig, contract-edge custody tests invoke git; after the conftest xdist guard and the acceptance `-m not serial_lean` filter, 1919/2139 pass hermetically and the residual failures are all tool-starved by construction. Completing this plane needs a hermetic-policy redesign (tool allowlist or lane partitioning) plus the deferred SC-11/SC-12/SC-36(d)/(e)/SC-37(c)-(f) cluster (owner Decision 1 pending); a partial or faked receipt is never acceptable. |
