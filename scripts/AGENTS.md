...
faces `manuscript/preamble.md` selects -- so it cannot re-run the acceptance and would otherwise run
nothing at all, which is exactly the audited state: a tested acceptance no
workflow invoked.
