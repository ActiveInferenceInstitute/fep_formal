...
[Open the offline interactive validation dashboard.]({{publication.repository_url}}/blob/{{source.commit}}/docs/formal-kernel-dashboard.html) Its filters expose all fifteen family diagnostics, while accessible tables retain the exact parameters, theorem mirrors, typed per-check relations and tolerances, and boundary observations without requiring a network connection.
